...
* @version 1.1
* Changed by hdb 26.2.2014, 14.5.2014, 16.10.2014
* 
...
/** 
 * Parameters that specify the robot finally after initialization.
 * Some have been assigned by the Server if id=0 was specified for 
 * the initialization of the agent. .
 * 
 * In future implementations they may be contained in a special class providing 
 * further information about the robot itself, e.g. by the vision perceptor.
 * Up to now, this information is not provided by LocalFieldView.
 ...
 */
   ...
   * the order of initialization. This allows to use the same agent program for
   * different players. Beam positions are then taken from util.BeamPoses.<br>
   * After initialization, this method receives some first messages from the server,
   * because the gyrometer and the accelerometer need the first server cycles
   * to even out. (actually suspended for speeding up -hdb)
   * <p/>
   ...
   *                      In case "0", the numbers are assigned by the server 
  ...
  public void initRobot(String robotID, String team, double beamCoordsX, double beamCoordsY, double beamCoordsRot) {
   ...
   sendBeamMessage();
     ...
     /** 
     * Beam the player to a given pose. 
     * Used at the beginning and after goals.
     * 
     * If player number was assigned by the server, the beam-message uses the poses
     * from class util.BeamPoses. Otherwise the parameters from calling initRobot are used.
     *  Changed by hdb 26.2.2014, 16.10.2014
     */
